...
export function VentureSnapshotTable({ ventures }) {
  return (
    <article className="min-w-0 overflow-hidden rounded-lg border border-slate-200 bg-white p-5 shadow-soft sm:p-6">
        ...
        <table className="w-full min-w-[760px] border-collapse text-left">
          <thead>
            <tr className="border-b border-slate-200 text-sm font-semibold text-slate-500">
              <th className="py-3 pr-5">Venture</th>
              <th className="py-3 pr-5">Stage</th>
              <th className="py-3 pr-5">Priority</th>
              <th className="py-3 pr-5 text-center">Pending</th>
          ...
          <tbody>
            {ventures.map((venture) => (
              <tr key={venture.name} className="border-b border-slate-100 last:border-0">
                <td className="py-4 pr-5 font-bold text-slate-950">{venture.name}</td>
                ...
                <td className="py-4 pr-5 text-center font-medium text-slate-900">{venture.pending}</td>
                <td className="max-w-[280px] py-4 text-slate-800">{venture.nextAction}</td>
              </tr>
            ))}
          </tbody>
        </table>
